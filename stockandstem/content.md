...
This project reinforced that **research vocabulary is interface copy.** The moment I replaced generic labels like "inventory item" with language cooks already used, task completion jumped—not because the layout changed, but because the system stopped feeling imported from another domain.
...
Stock & Stem is the case study I point to when explaining my librarian's eye for organization: structure only helps when it matches how people already sort the world.
